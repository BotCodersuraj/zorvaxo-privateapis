...
export default function Home(){
  ...
  const generateKey = async ()=>{
    ...
    const data = await res.json();
    if(data.success){ setApiKey(data.apiKey); setLimit(data.limit);}
    else alert(data.message);
  };
  ...
  if(!loggedIn) return (
    <div style={{padding:20}}>
      ...
      <p style={{cursor:"pointer",color:"blue"}} onClick={()=>setIsRegister(!isRegister)}>{isRegister?"Already have account? Login":"New user? Register"}</p>
    </div>
  );
